"""Lanzador LOCAL para probar sync_dolar_pedidos (Fase 2 - dolarización), sin cascada ni func start.

Uso:
  python run_dolar_pedidos_local.py               -> modo normal (una ejecución, exige LOAD_MODE_DOLAR_PEDIDOS='INCREMENTAL')
  python run_dolar_pedidos_local.py --historical   -> modo backfill (rondas de hasta 24 min con reanudación por checkpoint)
...
def leer_checkpoint():
    """Lee el checkpoint actual de Dolar Pedidos directo de SQL. Devuelve int o 0."""
    conn = pyodbc.connect(os.environ["SQL_AZURE_CONNECTION_STRING"])
    try:
        cursor = conn.cursor()
        cursor.execute("SELECT LastValue FROM Etl_Checkpoints WHERE KeyName = ?", 'checkpoint_dolar_pedidos_skip')
        row = cursor.fetchone()
...
def run_normal():
    ...
    try:
        print("=" * 70)
        print("[SEGURO] Prueba local de sync_dolar_pedidos")
        print(f"  Modo de carga (LOAD_MODE_DOLAR_PEDIDOS): {etl.LOAD_MODE_DOLAR_PEDIDOS}")
        print(f"  Tablero de versión (API_VERSION_PEDIDOS): {etl.API_VERSION_PEDIDOS}")
        print("=" * 70)

        assert etl.LOAD_MODE_DOLAR_PEDIDOS == 'INCREMENTAL', \
            "La primera prueba debe ser INCREMENTAL — aborta si está en HISTORICAL"

        start = time.time()
        resultado = etl.sync_dolar_pedidos()
        elapsed = time.time() - start

        print("=" * 70)
        print(f"[RESULTADO] Pedidos enriquecidos: {resultado}")
...
def run_historical():
    assert GesvisionEtl.LOAD_MODE_DOLAR_PEDIDOS == 'HISTORICAL', \
        "--historical requiere LOAD_MODE_DOLAR_PEDIDOS == 'HISTORICAL' en function_app.py"

    print("⚠️ BACKFILL COMPLETO (~36,000 pedidos, ~180 páginas). Se ejecutará en rondas de hasta 24 min con reanudación automática por checkpoint.")
    ...
    for ronda in range(1, 16):
        ...
        print(f"===== RONDA {ronda} FIN — enriquecidos en la ronda: {resultado} | checkpoint: {chk_despues} =====")
    ...
    print("=" * 70)
    print(f"[RESUMEN BACKFILL] Total pedidos enriquecidos: {total_general}")
