"""Lanzador LOCAL para probar sync_dolar_tasas (Fase 2 - dolarización), sin cascada ni func start.

Script simple: un solo MERGE SQL, sin paginación ni checkpoint (no hay rondas que reanudar).

Uso: python run_dolar_tasas_local.py
...
try:
    print("=" * 70)
    print("[SEGURO] Prueba local de sync_dolar_tasas")
    print("=" * 70)

    start = time.time()
    resultado = etl.sync_dolar_tasas()
    elapsed = time.time() - start

    print("=" * 70)
    print(f"[RESULTADO] Días con tasa registrados: {resultado}")
