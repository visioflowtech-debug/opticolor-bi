"""Lanzador LOCAL para probar sync_dolar_cobros (Fase 2 - dolarización), sin cascada ni func start.

Script simple: un solo UPDATE SQL, sin paginación ni checkpoint (pasada única).

Uso: python run_dolar_cobros_local.py
...
try:
    print("=" * 70)
    print("[SEGURO] Prueba local de sync_dolar_cobros")
    print("=" * 70)

    start = time.time()
    resultado = etl.sync_dolar_cobros()
    elapsed = time.time() - start

    print("=" * 70)
    print(f"[RESULTADO] Cobros dolarizados: {resultado}")
